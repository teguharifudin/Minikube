...
$(function() {
	$topicSlugName = getSlugNameFromLink(window.location.pathname);
});
...
$(window).scroll(function() {
    ...
    if ($(window).scrollTop() + $(window).height() >= $(document).height() - buffer) {
        ...
        getQuotesByTopic(page);
    }
});

function getQuotesByTopic(page) {
	$.ajax({
		type: "GET",
		url: "/quote/v1/quotes?topic=" +  $topicSlugName + "&page=" + page + "&per_page=",
